...
const ENTITIES = ['categories', 'products', 'restaurant_tables', 'customers', 'inventory', 'sales', 'payments', 'cash_movements', 'shifts', 'inventory_movements', 'suppliers', 'supplier_ledger', 'purchase_orders', 'returns', 'bundles', 'customer_ledger', 'store_credit_ledger', 'tax_profiles'];
...
function createSyncServer({ dbPath, port = 0, rateLimit = 60, onLog = () => {}, getPairingInfo = null, consumePairingCode = null, tls = null, host = null, allowPlaintextNonLoopback = false } = {}) {
  ...
  db.exec(`CREATE TABLE IF NOT EXISTS sync_records (
    ...
    revision INTEGER PRIMARY KEY AUTOINCREMENT, updated_at TEXT NOT NULL, UNIQUE(entity, entity_uuid)
  )`);
  ...
  const insertFresh = db.prepare(
    `INSERT INTO sync_records (entity,entity_uuid,branch_uuid,payload,updated_at) VALUES (?,?,?,?,?)`
  );
  function save(rec) {
    ...
    insertFresh.run(rec.entity, rec.uuid, GLOBAL_ENTITIES.has(rec.entity) ? null : rec.branch, rec.payload, rec.updated);
  }
  ...
  const requestHandler = async (req, res) => {
    ...
    try {
      ...
      const rows = db.prepare(`SELECT * FROM sync_records WHERE revision > ? AND (branch_uuid = ? OR branch_uuid IS NULL) ORDER BY revision`).all(cursor, branchUuid);
