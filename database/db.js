...
function createUpgradeSnapshot(reason = 'manual') {
  ...
  // نحذف WAL فقط بعد دمجه في pos.db، ثم ننسخ محتوى userData كاملاً باستثناء اللقطات
  // نفسها. لا يلامس هذا الإجراء ملفات العميل الأصلية.
  db.pragma('wal_checkpoint(TRUNCATE)');
  for (const entry of fs.readdirSync(userDataPath, { withFileTypes: true })) {
    if (entry.name === 'upgrade-backups') continue;
    fs.cpSync(path.join(userDataPath, entry.name), path.join(snapshotPath, entry.name), { recursive: true, force: false, errorOnExist: true });
  }
  ...
  fs.writeFileSync(path.join(snapshotPath, 'manifest.json'), JSON.stringify({
    ...
    included: 'userData except upgrade-backups',
  }, null, 2), 'utf8');
