 ...
 * @returns {Buffer}
 */
function imageToEscPosRaster(image, maxDotsWidth) {
 ...
 */
function buildFullPrintJob(image, { dotsWidth = 576, cut = true, feedLinesAfter = 3 } = {}) {
  const parts = [];
  parts.push(Buffer.from([ESC, 0x40])); // ESC @  تهيئة الطابعة
  parts.push(Buffer.from([ESC, 0x61, 0x01])); // ESC a 1  محاذاة للوسط
  parts.push(imageToEscPosRaster(image, dotsWidth));
...
async function captureWindowAndPrintNetwork(win, { ip, port, dotsWidth = 576 } = {}) {
  ...
  const jobBuffer = buildFullPrintJob(image, { dotsWidth });
