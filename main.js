...
const PUBLIC_IPC_CHANNELS = new Set([
  ...
  // هاتان القناتان لا تُعيدان أي بيانات حساسة (نبض/تسجيل حدث فقط)، وكل من الـ handler الخاص
  // بهما مكتوب أصلاً ليتجاهل الاستدعاء بأمان إذا currentUser فارغ (انظر `if (!currentUser) return ...`
  // بأسفل هذا الملف) — لكن هذا الغلاف كان يرمي الخطأ *قبل* وصول الاستدعاء لذلك الفحص الداخلي أصلاً،
  // فيغرق سجل الأخطاء برسائل "يجب تسجيل الدخول أولاً" في أي لحظة انتقال (خصوصاً حول تسجيل
  // الخروج، حيث تستمر مؤقّتات renderer بالعمل لحظات قبل إغلاق النافذة فعليًا).
  'system:rendererHeartbeat',
  'audit:clientEvent',
]);
...
// عمداً لكل الصلاحيات (انظر setPermissionRequestHandler)، فتفشل بصمت. هذه القناة لا
// تقرأ الحافظة أبداً ولا تحتاج تسجيل دخول، فهي متاحة أيضاً من شاشة تفعيل الترخيص.
ipcMain.handle('system:copyToClipboard', (_event, text) => {
