...
function renderTable(customers) {
  ...
  for (const c of customers) {
    ...
    tr.innerHTML = `
      <td>${escapeHtml(c.name || t('common.noName', 'بدون اسم'))}</td>
      ...
      <td class="row-actions"><button class="btn btn-secondary btn-sm" data-action="ledger">${t('customers.statement', 'كشف حساب')}</button> <button class="btn btn-secondary btn-sm" data-action="edit">${t('common.edit', 'تعديل')}</button></td>
    `;
  ...
  document.getElementById('ledgerBalances').innerHTML = `
    <div class="kpi-row" style="margin-bottom:14px;">
      <div class="kpi-card" data-tone="${debt > 0 ? 'amber' : 'slate'}">
        ...
        <div class="kpi-body"><div class="kpi-label">${t('customers.outstandingDebt')}</div><div class="kpi-value">${debt.toFixed(2)}</div></div>
      </div>
      <div class="kpi-card" data-tone="${storeCredit > 0 ? 'indigo' : 'slate'}">
        ...
        <div class="kpi-body"><div class="kpi-label">${t('customers.storeCreditOwed')}</div><div class="kpi-value">${storeCredit.toFixed(2)}</div></div>
      </div>
    </div>
  `;
  const rows = await window.api.customers.ledger(customer.id);
  const host = document.getElementById('ledgerRows');
  host.innerHTML = rows.length ? rows.map((row) => `<div class="breakdown-row"><div class="breakdown-label"><span>${ledgerEntryLabel(row)}</span><span>${new Date(row.created_at.replace(' ', 'T') + 'Z').toLocaleDateString('ar')}</span></div><div class="breakdown-label"><span>${Number(row.amount).toFixed(2)}</span><span>${ts('الرصيد')}: ${Number(row.balance_after).toFixed(2)}</span></div></div>`).join('') : `<div class="empty-state">${t('customers.noLedgerMovements')}</div>`;
...
document.getElementById('receivePaymentBtn').addEventListener('click', () => {
  if (!ledgerCustomer || !(Number(ledgerCustomer.balance) > 0)) return showToast(ts('لا يوجد رصيد مستحق على هذا العميل.'), 'error');
...
document.getElementById('debtPaymentForm').addEventListener('submit', async (event) => {
  ...
  try {
    ...
    await loadCustomers();
    if (result.cashMovementRecorded === false && document.getElementById('debtPaymentMethod').value === 'cash') await infoDialog(t('customers.paymentRecordedNoOpenSession'));
  } catch (error) { showToast(ts('تعذر تسجيل التسديد: ') + error.message, 'error'); }
});
...
function openModal(c = null) {
  ...
  customerForm.reset();
  fieldPoints.value = 0;
  modalTitle.textContent = c ? t('customers.editCustomer') : t('customers.newCustomer');
  if (c) {
