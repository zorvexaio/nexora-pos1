let suppliers = []; let products = []; let purchaseItems = []; let editingSupplier = null; let currentBranch = null;
...
function renderSuppliers() { $('suppliersBody').innerHTML = suppliers.map((s) => `<tr><td>${esc(s.name)}</td><td>${esc(s.phone || '—')}</td><td>${Number(s.balance).toFixed(2)}</td><td><button class="btn btn-secondary btn-sm" data-id="${s.id}">تعديل</button></td></tr>`).join(''); $('suppliersBody').querySelectorAll('button').forEach((b) => b.addEventListener('click', () => openSupplier(suppliers.find((s) => s.id === Number(b.dataset.id))))); }
